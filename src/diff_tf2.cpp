...
Odometry_calc::Odometry_calc()
{
	...
	//r_wheel_sub = n.subscribe("/rwheel",10, &Odometry_calc::rightencoderCb, this);
        twist_msg_sub = n.subscribe("/arduino_vel",1000,&Odometry_calc::twist_odom_callback, this);
	...
	if ( now > t_next) 
		...
		dr = th / elapsed; // wd odom
                */ 
                ...
                //geometry_msgs::Quaternion odom_quat = tf::createQuaternionFromYaw(theta_final);
		...
		odom_trans.transform.rotation = tf::createQuaternionMsgFromYaw(theta_final);
                odom_broadcaster.sendTransform(odom_trans); // Don't publish wheel odometry transform when AMCL is being used.
...
void Odometry_calc::twist_odom_callback(const geometry_msgs::Twist::ConstPtr& msg)
{
   vd = msg->linear.x ;
   wd = msg->angular.z ;
}
